...
*Chapter 3 - Formatted Input/Output - Project 3.
*File created: 12/08/2025
*File last modified: 12/08/2025
*Name: ISBN_program.c
*Purpose: Books are identified by an International Standard Book Number (ISBN)
		  ISBNs assigned after January 1. 2007 contain 13 digits, arranged in five groups,
		  such as 978-0-393-97950-3
		  (Older ISBNs use 10 digits) The first group (the GS1 prefix) is currently either
		  978 or 979
		  The group identifier specifies the language or country of origin (for example,
		  0 and 1 are used in English-speaking countries)
		  ...
		  that groups have the same lengths shown in this example
		  Test your program with actual ISBN values (usually found on the back
...
int main(void)
{
	...
	int gs1_prefix = 0, group_identifier = 0;
	int publisher_code = 0, item_number = 0;
	int check_digit = 0;
